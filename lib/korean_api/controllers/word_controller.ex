defmodule KoreanApi.Controllers.WordController do
  ...
  def get("korean=eq." <> parameters) do
    ...
    # If it doesn't work to translate the word, try the stem
    stem =
      word
    ...
    with :not_found <- get_from_database(word, other_parameters),
         ...
         :not_found <- get_from_database(stem, other_parameters),
         :not_found <- get_from_krdict(stem, other_parameters) do
      []
    end
  end
  ...
  defp get_from_krdict(korean, other_parameters) when is_list(other_parameters) do
    ...
    task_korean_example_sentences =
      Task.async(fn -> KoreanDictionary.korean_example_sentences(korean) end)

    with {:ok, word} <-
           KoreanDictionaryService.korean_to_english(
             korean,
             Task.await(task_korean_to_english, 50_000)
           ),
         :ok <-
           KoreanDictionaryService.korean_to_korean(
             korean,
             word,
             Task.await(task_korean_to_korean, 50_000)
           ),
         :ok <-
  ...
  defp get_missing_translations([translation]) do
    ...
    if Map.get(translation, "word_korean_explanations") == [] do
      KoreanDictionaryService.korean_to_korean(
        korean,
        word,
        KoreanDictionary.korean_to_korean(korean)
      )
    end
